...
ssize_t vfs_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	...
	// 
	int xattr_len = vfs_getxattr(file_mnt_idmap(file), file->f_path.dentry, "user.cw3_encrypt", xattr_value, sizeof(xattr_value));
	if (xattr_len > 0) {
		xattr_key = (unsigned char)simple_strtoul(xattr_value, NULL, 10);
	}
	if (xattr_key > 0) {
		for (ssize_t i = 0; i < ret; i++) {
			kbuf[i] = buf[i] ^ xattr_key;
		}
