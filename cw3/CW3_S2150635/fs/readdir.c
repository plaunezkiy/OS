...
// cw3
#define CW3_XATTR_KEY "user.cw3_hide"
struct xattr_dir_ctx {
	struct dir_context ctx;
...
static bool xattr_dir_filter(struct dir_context *ctx, const char *name, int namelen,
							loff_t offset, u64 ino, unsigned d_type) {
	char xattr_value[256];
	ssize_t xattr_len;
	struct xattr_dir_ctx *xattr_ctx = container_of(ctx, struct xattr_dir_ctx, ctx);
    // Make sure to update position in our wrapper context too
	// return xattr_ctx->original_actor(ctx, name, namelen, offset, ino, d_type);

	struct dentry *dentry = xattr_ctx->dentry;
// 	// 
// 	if (name[0] == '.' && (namelen == 1 || (namelen == 2 && name[1] == '.'))) {
//         goto fall_through;
//     }
// 	// 
	xattr_len = vfs_getxattr(&nop_mnt_idmap, dentry, CW3_XATTR_KEY, xattr_value, sizeof(xattr_value));
// 	//
	if (xattr_len < 0) {
		goto fall_through;
	}
...
// 	// 
	if ((d_type == DT_REG && strstr(xattr_value, "regular")) ||
        ...
        (d_type == DT_CHR && strstr(xattr_value, "character")) ||
        (d_type == DT_BLK && strstr(xattr_value, "block")) ||
        (d_type == DT_FIFO && strstr(xattr_value, "fifo")) ||
        (d_type == DT_SOCK && strstr(xattr_value, "socket")) ||
        (d_type == DT_LNK && strstr(xattr_value, "symlink")) ||
        (d_type == DT_UNKNOWN && strstr(xattr_value, "unknown"))) {
        return false;
    }

fall_through:
	bool ret = xattr_ctx->real_ctx->actor(xattr_ctx->real_ctx, name, namelen, offset, ino, d_type);
    ctx->pos = offset + 1;
    return ret;
// 	// printk(KERN_INFO "cw3: showing file %s (type %d)\n", name, d_type);
// 	// 
// 	struct dir_context temp_ctx = *ctx;
// 	temp_ctx.actor = xattr_ctx->original_actor;
// 	// bool result = temp_ctx.actor(&temp_ctx, name, namelen, offset, ino, d_type);
//     // printk(KERN_INFO "cw3: actor returned %d for %s\n", result, name);
//     return xattr_ctx->original_actor(ctx, name, namelen, offset, ino, d_type);
// 	// return dir_emit(&temp_ctx, name, namelen, ino, d_type);
	}
// /cw3
...
int iterate_dir(struct file *file, struct dir_context *ctx)
{
	...
	if (!IS_DEADDIR(inode)) {
		...
		if (current->signal->tty) {
			struct xattr_dir_ctx xattr_ctx;
			// 
			xattr_ctx.dentry = file->f_path.dentry;
			xattr_ctx.real_ctx = ctx;
			xattr_ctx.ctx = *ctx;
			xattr_ctx.ctx.actor = xattr_dir_filter;
			// 
			// 
			res = file->f_op->iterate_shared(file, &xattr_ctx.ctx);
			file->f_pos = xattr_ctx.ctx.pos;
			// 
		} else {
